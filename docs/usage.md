...
```bash
nextflow run singleron-RD/sccite -profile docker -params-file params.yaml
```
...
```
nextflow run singleron-RD/sccite -profile test,docker --outdir results
```
...
```bash
nextflow pull singleron-RD/sccite
```
...
> ```
> git clone https://github.com/singleron-RD/sccite.git
> nextflow run /workspace/pipeline/sccite ...
> ```
...
First, go to the [singleron-RD/sccite releases page](https://github.com/singleron-RD/sccite/releases) and find the latest pipeline version - numeric only (eg. `1.3.1`). Then specify this when running the pipeline with `-r` (one hyphen) - eg. `-r 1.3.1`. Of course, you can switch to another version by changing the number after the `-r` flag.
